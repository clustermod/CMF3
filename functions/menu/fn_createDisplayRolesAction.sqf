...
if !(isNil{([(ace_interact_menu_ActSelfNamespace getVariable (typeOf player)), ["ACE_SelfActions","EMF_BUGACTIONS", "emf_display_roles"]] call ace_interact_menu_fnc_findActionNode)}) exitWith {};

_rolesAction = ["emf_display_roles", "Show roles", "rsc\data\icon_ace_team_ca.paa", {
  private _roles = [];
  {
    private _p = _x;
