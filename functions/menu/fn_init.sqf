...
private _childrenSelf = {
    ...
    private _action = ([QGVAR(safestart_menu), "Disable Safestart", "rsc\data\icon_ace_startgame_ca.paa", {}, { !(missionNamespace getVariable [QEGVAR(gameplay,safestart_disable), false]) }, {
        private _actions = [];
    ...
    private _action = ([QGVAR(submitAAR), "Submit After Action Report", "rsc\data\icon_ace_debrief_ca.paa", { _this call FUNC(submitReportAction) }, { true }] call ace_interact_menu_fnc_createAction);
    _actions pushBack [_action, [], _target];

    /* Hide HUD Action */
    private _action = ([QGVAR(toggleHUD), "Toggle HUD", "rsc\data\icon_viewDistance_ca.paa", { _this call FUNC(toggleHUDAction) }, { true }] call ace_interact_menu_fnc_createAction);
    _actions pushBack [_action, [], _target];

    /* Open View Distance Action */
    private _action = ([QGVAR(viewDistance), "View Distance Settings","rsc\data\icon_viewDistance_ca.paa", { _this call FUNC(viewDistanceAction) }, { true }] call ace_interact_menu_fnc_createAction);
    _actions pushBack [_action, [], _target];
