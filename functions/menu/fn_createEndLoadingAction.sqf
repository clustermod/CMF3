...
if !(isNil{([(ace_interact_menu_ActSelfNamespace getVariable (typeOf player)), ["ACE_SelfActions","EMF_BUGACTIONS", "emf_endLoadingScreen"]] call ace_interact_menu_fnc_findActionNode)}) exitWith {};

_fixAction = ["emf_endLoadingScreen", "Fix ""stuck loading"" bug", "rsc\data\icon_bug_ca.paa", {
  private _onConfirm = {
    params["_value"];
