...
if !(isNil{([(ace_interact_menu_ActSelfNamespace getVariable (typeOf player)), ["ACE_SelfActions","EMF_BUGACTIONS", "emf_start_game_category"]] call ace_interact_menu_fnc_findActionNode)}) exitWith {};

_startCategoyAction = ["emf_start_game_category", "Start Game", "rsc\data\icon_startGame_ca.paa", {}, {(!(missionNamespace getVariable ["EMF_missionSafeStart", false]) && !isNull (getAssignedCuratorLogic player)) || ((call BIS_fnc_admin) > 0 && !(missionNamespace getVariable ["EMF_missionSafeStart", false]))}] call ace_interact_menu_fnc_createAction;

_startInstantAction = ["emf_start_game_instant", "Instant", "rsc\data\icon_instant_ca.paa", {
...
_startDelayedAction = ["emf_start_game_delayed", "Delayed", "rsc\data\icon_delayed_ca.paa", {
  [] spawn {
