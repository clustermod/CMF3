...
if (typeName _respawns != "SCALAR") exitWith {["ERR", format['Object must be type "NUMBER", type %1 supplied', (typeName _respawns)], "init", "limitedRespawns"] call EMF_DEBUG; false;};

// If player is not in supplied team don't run
if (player getVariable ["emf_utilities_setRole_team", 0] == _teamLimit) then {
