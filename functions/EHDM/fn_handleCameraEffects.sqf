 ...
 *
 * Example:
 * [this, [p1, p2], 0.3] call EMF_fnc_handleCameraEffects
 *
 * Public: No
 */
...
if ((_crew find player) >= 0) then {
	...
	[] spawn {
		if (vehicle player == player || player getVariable ["EMF_SmokeAssigned", false]) exitWith {};
		private _v = vehicle player;
		private _pp = ppEffectCreate ["colorCorrections", 1501];
		private _c = 0;

		player setVariable ["EMF_SmokeAssigned", true];
		...
		player setVariable ["EMF_SmokeAssigned", false];
	...
	if (_playerDamage != 3) then {
		[_playerDamage] call EMF_fnc_postProcess;
		[_playerDamage] call EMF_fnc_handleMedical;
	} else {
		[0] call EMF_fnc_postProcess;
	};
