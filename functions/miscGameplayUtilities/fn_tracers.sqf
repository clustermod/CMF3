EMF_fnc_T_applyTracer =
{
	if (!isServer) exitWith {
		['Function not called from server'] call BIS_fnc_error;
		['Function not called from server'] call BIS_fnc_log;
		false;
	};
...
switch (typeName _unit) do {
    case ("SIDE"): {
		...
		default {
			['Unit must be type "OBJECT", "ARRAY" or "SIDE", type %1 supplied', (typeName _unit)] call BIS_fnc_error;
			['Unit must be type "OBJECT", "ARRAY" or "SIDE", type %1 supplied', (typeName _unit)] call BIS_fnc_log;
			false;
		};
};
