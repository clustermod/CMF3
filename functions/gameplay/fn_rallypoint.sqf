 ...
 *
 * Example:
 * [this, 5] call emf_gameplay_fnc_rallypoint
 *
 * public: Yes
*/
scriptName "functions\gameplay\fn_rallypoint.sqf";
...
if (isNil "_unit") exitWith {["ERR", format['Unit is not set', (typeName _unit)], "rallypoint", "gameplay"] call EMF_DEBUG};
...
switch (typeName _unit) do {
    case ("ARRAY"): {
		...
		case ("STRING"): {
			...
			if ((player getVariable ["emf_utilities_setRole_role", "RFL"]) == _unit && isNull (_hasRally select 0)) then {
		...
		default {
			["ERR", format['Unit must be type "OBJECT", "ARRAY" or "STRING", type %1 supplied', (typeName _unit)], "rallypoint", "gameplay"] call EMF_DEBUG;
				false;
		};
};
