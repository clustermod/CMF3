...
_this spawn {
			...
			[] spawn {
				while {!(missionNamespace getVariable [QGVAR(safestart_disable), false])} do {
					...
					_safestartHint = _safestartHint + format [
						...
						(_phase select 0),
						[(EGVAR(main,unifiedTime) - _startTime), 'MM:SS'] call BIS_fnc_secondsToString
					];

					/* Cutoffs */
					private _cutoffPhases = missionNameSpace getVariable [QGVAR(safestart_phaseCutOffs), []];
					{
						if ((_x select 1) > ((EGVAR(main,unifiedTime) - _startTime))) then {
							_safestartHint = _safestartHint + format [
								"<t align='left' color='#888888' size='0.95'>%1 Cut-off</t> <t align='right' color='#888888' size='0.95'>%2</t><br/>",
								(_x select 0),
								[(_x select 1) - (EGVAR(main,unifiedTime)), 'MM:SS'] call BIS_fnc_secondsToString
							];
						} else {
	...
	/* Show mission gamemode and name */
	sleep 3;
	[[], {
		private _missionData = missionNameSpace getVariable [QEGVAR(common,missionData), []];
		private _hash = [_missionData] call CBA_fnc_hashCreate;
