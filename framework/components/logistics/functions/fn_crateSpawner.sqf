...
_this spawn {
	...
	/* Wait until a preset is loaded */
	waitUntil { !isNil { missionNamespace getVariable QGVAR(crateConfig) } };

	/* Create Logistics root menu */
	private _action = [QGVAR(root), "Logistics", "", {}, { true }] call ace_interact_menu_fnc_createAction;
	...
	private _config = missionNamespace getVariable [QGVAR(crateConfig), [] call CBA_fnc_hashCreate];
	{
		...
		private _action = [
			...
			"", 
			{
				...
				missionNamespace setVariable [QGVAR(crateHash), _newHash, true];
			}, 
		...
		[_obj, 0, ["ACE_MainActions", QGVAR(root)], _action] call ace_interact_menu_fnc_addActionToObject;
